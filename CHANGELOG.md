...
- Version: `0.2.14-task-center-policies.20260623+build.1`
- App 通知规则：新增独立的 App 通知策略服务与管理页，把仓库钥匙、钥匙上传 SLA、日终交接、补货、挂钥匙等事件切到 `policyKey` 驱动的收件人规则。
- 任务中心：顶部“未安排”按当前页面显示的清洁任务和线下其他任务重新统计，并列出未安排任务；检查安排中的 `self_complete` 改为“已检查”，纯入住任务在该场景下不再强制分配检查人员。
- 任务展示：Web 任务中心和清洁日历统一到同一套语义色板，延后检查统一显示“延后检查”，不再继续混用旧标签颜色或把延后任务写成“退房/入住”。
- 入住检查：新增 `inspection_scope`，可区分“仅改密码”和“检查后挂钥匙”，并把该字段透传到任务中心和移动端任务数据。
- 房源：房源列表支持整行打开详情抽屉，房源创建/编辑/详情补齐 Wi-Fi 用户名和密码字段。
