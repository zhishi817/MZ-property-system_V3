...
exports.router.get('/tasks', (req, res) => {
    ...
    if (!supabase_1.hasSupabase) {
        if (date)
            return res.json(store_1.db.cleaningTasks.filter((t) => t.date === date));
...
exports.router.get('/staff', (req, res) => {
...
const patchSchema = zod_1.z.object({ scheduled_at: zod_1.z.string().optional(), status: zod_1.z.enum(['pending', 'scheduled', 'done']).optional() });
...
exports.router.get('/order/:orderId', (req, res) => {
    const order = store_1.db.orders.find(o => o.id === req.params.orderId);
    if (!order)
        return res.status(404).json({ message: 'order not found' });
    const tasks = store_1.db.cleaningTasks.filter(t => t.property_id === order.property_id && t.date === (order.checkout || t.date));
    res.json(tasks);
});
exports.router.get('/capacity', (req, res) => {
    const { date } = req.query;
    const dateStr = date || new Date().toISOString().slice(0, 10);
    const result = store_1.db.cleaners.map(c => {
        const assigned = store_1.db.cleaningTasks.filter(t => t.date === dateStr && t.assignee_id === c.id).length;
