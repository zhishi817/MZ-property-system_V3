...
-- APPLY: run the preview SQL first and review all result sets before executing this file.
BEGIN;
...
CREATE TEMP TABLE late_checkout_ready ON COMMIT DROP AS
SELECT *
FROM late_checkout_resolution
WHERE resolution = 'ready';

CREATE TEMP TABLE late_checkout_inserted_tx ON COMMIT DROP AS
WITH inserted AS (
  ...
  SELECT
    'late-checkout-20260105-ft-' || md5(ready.order_id),
    'income',
    ready.amount,
    'AUD',
    'order',
    ready.order_id,
    ready.checkout::date,
    'Late checkout income - historical backfill 2026-06-06',
    'late_checkout',
    ready.property_id
  FROM late_checkout_ready ready
  WHERE NOT EXISTS (
    SELECT 1
    FROM finance_transactions existing
    WHERE existing.kind = 'income'
      AND lower(coalesce(existing.category, '')) = 'late_checkout'
      AND existing.ref_type = 'order'
      AND existing.ref_id = ready.order_id
  )
...
CREATE TEMP TABLE late_checkout_inserted_company_income ON COMMIT DROP AS
WITH inserted AS (
  ...
  SELECT
    'late-checkout-20260105-ci-' || md5(ready.order_id),
    ready.checkout::date,
    ready.amount,
    'AUD',
    'late_checkout',
    ...
    ready.order_id
  FROM late_checkout_ready ready
  JOIN late_checkout_inserted_tx tx
    ON tx.ref_id = ready.order_id
  WHERE NOT EXISTS (
    SELECT 1
    FROM company_incomes existing
    WHERE lower(coalesce(existing.category, '')) = 'late_checkout'
      AND existing.ref_type = 'order'
      AND existing.ref_id = ready.order_id
  )
  ON CONFLICT (category, ref_type, ref_id) DO NOTHING
  RETURNING *
)
SELECT * FROM inserted;
...
DECLARE
  ...
  tx_total numeric(12,2);
  income_total numeric(12,2);
BEGIN
  SELECT count(*), round(coalesce(sum(amount), 0), 2)
    INTO tx_count, tx_total
    FROM late_checkout_inserted_tx;
  SELECT count(*), round(coalesce(sum(amount), 0), 2)
    INTO income_count, income_total
    FROM late_checkout_inserted_company_income;

  IF tx_count <> income_count OR tx_total <> income_total THEN
    RAISE EXCEPTION
      'Mirror validation failed: finance count/total %/% company count/total %/%',
      tx_count, tx_total, income_count, income_total;
  END IF;
END
$mirror_validation$;
...
SELECT
  'late-checkout-20260105-audit-ft-' || md5(tx.id),
  'FinanceTransaction',
  tx.id,
  'create',
  'sql-backfill-2026-06-06',
  NULL,
  to_jsonb(tx),
  now()
FROM late_checkout_inserted_tx tx
ON CONFLICT (id) DO NOTHING;
...
SELECT
  'late-checkout-20260105-audit-ci-' || md5(income.id),
  'CompanyIncome',
  income.id,
  'create',
  'sql-backfill-2026-06-06',
  ...
  to_jsonb(income),
  now()
FROM late_checkout_inserted_company_income income
ON CONFLICT (id) DO NOTHING;

SELECT
  (SELECT count(*) FROM late_checkout_source) AS source_count,
  (SELECT round(sum(amount), 2) FROM late_checkout_source) AS source_amount,
  (SELECT count(*) FROM late_checkout_inserted_tx) AS inserted_finance_count,
  (SELECT round(coalesce(sum(amount), 0), 2) FROM late_checkout_inserted_tx) AS inserted_finance_amount,
  (SELECT count(*) FROM late_checkout_inserted_company_income) AS inserted_company_count,
  (SELECT round(coalesce(sum(amount), 0), 2) FROM late_checkout_inserted_company_income) AS inserted_company_amount;
