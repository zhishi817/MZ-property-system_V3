...
async function main() {
  ...
  assert.match(route, /const feedbackMediaRows = matchingGuestLuggageRows\.length > 0 \|\| !hasTaskOrDayEndMedia\s*\? await findPropertyFeedbackMediaRows\(pgPool, key\)/, 'a temporary-notice association must not suppress feedback or external-maintenance collision detection, while an ambiguous task/day-end source remains fail closed')
