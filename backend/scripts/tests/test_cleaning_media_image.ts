...
async function main() {
  ...
  assert.match(route, /jsonb_array_elements_text\(w\.photo_urls\) AS stored\(value\)/, 'offline media must compare canonical identities from exact stored photo references')
  assert.match(route, /mzappTaskPhotoReferenceVariants\(requestedKey\)[\s\S]*mzappTaskPhotoReferenceVariants\(sourceUrl\)/, 'key and source URL must independently resolve offline ownership before generic feedback lookup')
