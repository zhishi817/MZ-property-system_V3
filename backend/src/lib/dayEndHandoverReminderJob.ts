...
async function listPendingDayEndHandoverUserIds(date: string) {
  ...
  return Array.from(new Set((result?.rows || []).map((row: any) => String(row.user_id || '').trim()).filter(Boolean)))
}
...
export async function runDayEndHandoverReminder(params?: { at?: string; date?: string }) {
  ...
  const userIds = await listPendingDayEndHandoverUserIds(date)
