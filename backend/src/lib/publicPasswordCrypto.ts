...
function readKey(): Buffer | null {
  const raw = String(process.env.PUBLIC_ACCESS_PASSWORD_ENC_KEY || '').trim()
  if (!raw) return null
  try {
...
export function hasPublicAccessPasswordKey(): boolean {
  return !!readKey()
}

export function encryptPublicAccessPassword(plain: string): string | null {
  const key = readKey()
  if (!key) return null
...
export function decryptPublicAccessPassword(enc: string): string | null {
  const key = readKey()
  if (!key) return null
