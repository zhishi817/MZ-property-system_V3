...
function effectiveWorkTaskStatus(value: any, assigneeId?: any): string {
  const status = normStatus(value)
  return status === 'todo' && String(assigneeId ?? '').trim() ? 'assigned' : status
}
...
async function canSubmitMzappSelfCompleteLockboxVideo(user: any, row: any, userId: string) {
  if (effectiveInspectionMode(row) !== 'self_complete') return false
  const cleanerId = String(row?.cleaner_id || '').trim()
  const assigneeId = String(row?.assignee_id || '').trim()
  if (userId && (cleanerId === userId || assigneeId === userId)) return true
  return userHasManualWorkTaskAction(user, userId, 'cleaning_tasks', String(row?.id || '').trim(), 'upload_access_video')
}
...
async function canSubmitMzappSelfCompleteRestock(user: any, row: any, userId: string) {
  if (effectiveInspectionMode(row) !== 'self_complete') return false
  const cleanerId = String(row?.cleaner_id || '').trim()
  const assigneeId = String(row?.assignee_id || '').trim()
  if (userId && (cleanerId === userId || assigneeId === userId)) return true
  if (await userHasManualWorkTaskAction(user, userId, 'cleaning_tasks', String(row?.id || '').trim(), 'fill_supplies')) return true
  return userHasManualWorkTaskAction(user, userId, 'cleaning_tasks', String(row?.id || '').trim(), 'complete_cleaning')
}

async function canSubmitMzappRestockProof(user: any, row: any, userId: string) {
  if (await canSubmitMzappInspection(user, row, userId)) return true
  return canSubmitMzappSelfCompleteRestock(user, row, userId)
}

async function canViewMzappRestockProof(user: any, row: any, userId: string) {
  if (canViewAll(user)) return true
  return canSubmitMzappRestockProof(user, row, userId)
}
...
function parseFormPhotoSourceIds(raw: any) {
...
function formPhotoTaskSelectSql() {
...
async function resolveFormPhotoTaskRows(workTaskRef: string, sourceIdHints: string[]) {
  if (!hasPg || !pgPool) return [] as FormPhotoTaskRow[]
  const ref = String(workTaskRef || '').trim()
  if (!ref) return [] as FormPhotoTaskRow[]

  let sql = ''
  let values: any[] = []
  const merged = ref.match(/^cleaning_tasks_merged:([^:]+):(.+)$/)
  const turnover = ref.match(/^cleaning_tasks_[^:]+_turnover:([^:]+):([^:]+):/)
  const grouped = ref.match(/^cleaning_tasks_[^:]+:(.+)$/)

  if (merged) {
    sql = `${formPhotoTaskSelectSql()}
       WHERE COALESCE(t.task_date, t.date)::date = $1::date
         AND (UPPER(COALESCE(p_id.code::text, p_code.code::text, t.property_id::text)) = UPPER($2)
              OR COALESCE(p_id.id::text, p_code.id::text, t.property_id::text) = $2)`
    values = [String(merged[1]), String(merged[2])]
  } else if (turnover) {
    const propertyId = String(turnover[2] || '').trim()
    if (propertyId && propertyId !== 'unknown') {
      sql = `${formPhotoTaskSelectSql()}
         WHERE COALESCE(t.task_date, t.date)::date = $1::date
           AND COALESCE(p_id.id::text, p_code.id::text, t.property_id::text) = $2`
      values = [String(turnover[1]), propertyId]
    } else if (sourceIdHints.length) {
      sql = `${formPhotoTaskSelectSql()}
         WHERE t.id::text = ANY($1::text[])`
      values = [sourceIdHints]
    }
  } else if (grouped) {
    const ids = parseFormPhotoSourceIds(grouped[1])
    if (ids.length) {
      sql = `${formPhotoTaskSelectSql()}
         WHERE t.id::text = ANY($1::text[])`
      values = [ids]
  ...
  if (!sql) return [] as FormPhotoTaskRow[]
  const result = await pgPool.query(sql, values)
  const rows = (result?.rows || []).map((row: any) => ({
    id: String(row.id || '').trim(),
    task_date: row.task_date == null ? null : String(row.task_date).slice(0, 10),
    property_id: row.property_id == null ? null : String(row.property_id).trim() || null,
    property_code: row.property_code == null ? null : String(row.property_code).trim() || null,
    inspector_id: row.inspector_id == null ? null : String(row.inspector_id).trim() || null,
    cleaner_id: row.cleaner_id == null ? null : String(row.cleaner_id).trim() || null,
    assignee_id: row.assignee_id == null ? null : String(row.assignee_id).trim() || null,
  })).filter((row: FormPhotoTaskRow) => !!row.id)

  return rows
}

export async function canViewFormPhotoTaskRows(user: any, rows: FormPhotoTaskRow[], userId: string) {
  if (!rows.length) return false
  if (canViewAll(user)) return true
  const uid = String(userId || '').trim()
  if (!uid) return false
  if (rows.some((row) => [row.inspector_id, row.cleaner_id, row.assignee_id].some((id) => String(id || '').trim() === uid))) return true
  if (!hasPg || !pgPool) return false
  ...
  const viewActions = new Set(['*', 'fill_supplies', 'submit_inspection', 'complete_cleaning', 'upload_key_photo', 'upload_access_video', 'report_issue'])
  return (result?.rows || []).some((row: any) => normalizeParticipantActionIds(row?.action_ids).some((action) => viewActions.has(action)))
}

export function formPhotoMediaType(type: string) {
  if (type.startsWith('inspection_') && type !== 'inspection_consumables_confirmed') return { source: 'inspection', area: type.slice('inspection_'.length) }
  if (type.startsWith('restock_proof:')) return { source: 'restock', item_id: type.slice('restock_proof:'.length) }
  if (type === 'consumable_living_room_photo') return { source: 'consumable', area: 'living', label: '客厅' }
  if (type === 'issue_photo') return { source: 'issue', area: 'unclean', label: '问题照片' }
  return null
}

function formPhotoMeta(note: any) {
  try {
...
async function loadFormPhotoRecords(taskRows: FormPhotoTaskRow[]) {
  if (!hasPg || !pgPool || !taskRows.length) return []
  const taskIds = taskRows.map((row) => row.id)
  const mediaResult = await pgPool.query(
    `SELECT id::text AS id, task_id::text AS task_id, type, url, note, captured_at, created_at, uploader_id::text AS uploader_id
       FROM cleaning_task_media
      WHERE task_id::text = ANY($1::text[])
        AND COALESCE(url, '') <> ''
        AND (type LIKE 'inspection_%' OR type LIKE 'restock_proof:%' OR type IN ('consumable_living_room_photo', 'issue_photo'))
      ORDER BY captured_at ASC NULLS LAST, created_at ASC NULLS LAST, id ASC`,
    [taskIds],
  )
  const usageResult = await pgPool.query(
    `SELECT id::text AS id, task_id::text AS task_id, item_id::text AS item_id, item_label, photo_url, photo_urls, created_at
  ...
  for (const row of mediaResult?.rows || []) {
    ...
    const mapped = formPhotoMediaType(type)
    if (!mapped) continue
    const meta = formPhotoMeta(row.note)
    records.push({
      id: String(row.id || '').trim(),
      task_id: String(row.task_id || '').trim(),
      ...mapped,
      item_id: mapped.item_id || null,
      label: mapped.label || (meta?.label ? String(meta.label) : mapped.item_id || null),
      url: String(row.url || '').trim(),
      uploaded_key: null,
      captured_at: row.captured_at ? String(row.captured_at) : null,
      created_at: row.created_at ? String(row.created_at) : null,
      uploader_id: row.uploader_id == null ? null : String(row.uploader_id).trim() || null,
      status: row.uploader_id ? 'synced' : 'synced_unlabeled',
    })
  }
  for (const row of usageResult?.rows || []) {
    const urls = normalizeStoredPhotoUrls(row.photo_urls, row.photo_url)
    for (const [index, url] of urls.entries()) {
      records.push({
        id: `${String(row.id || '').trim()}:${index}`,
        task_id: String(row.task_id || '').trim(),
        source: 'consumable',
        item_id: String(row.item_id || '').trim() || null,
        label: String(row.item_label || row.item_id || '').trim() || null,
        url,
        uploaded_key: null,
        captured_at: null,
        created_at: row.created_at ? String(row.created_at) : null,
        uploader_id: null,
        status: 'synced_unlabeled',
      })
    }
  }
  ...
  try {
    ...
    const transactionResult = await pgRunInTransaction(async (client) => {
      ...
      const actionResult = await applyCleaningTaskActionTransition({
        ...
        metadata: {
          route: 'mzapp.cleaning_tasks.lockbox_video',
...
router.get('/work-tasks/:id/form-photos', async (req, res) => {
  const user = (req as any).user
  if (!user) return res.status(401).json({ message: 'unauthorized' })
  const workTaskRef = String(req.params.id || '').trim()
  const sourceIdHints = parseFormPhotoSourceIds((req.query as any)?.source_ids)
  const userId = String(user.sub || '').trim()
  if (!workTaskRef) return res.status(400).json({ message: 'missing work task reference' })
  if (!hasPg || !pgPool) return res.status(500).json({ message: 'pg not available' })
  try {
    const taskRows = await resolveFormPhotoTaskRows(workTaskRef, sourceIdHints)
    if (!taskRows.length) return res.status(404).json({ message: 'not found' })
    if (!await canViewFormPhotoTaskRows(user, taskRows, userId)) return res.status(403).json({ message: 'forbidden' })
    const items = await loadFormPhotoRecords(taskRows)
    return res.json({ task_ids: taskRows.map((row) => row.id), items })
  } catch (e: any) {
    return res.status(500).json({ message: e?.message || 'form_photos_failed' })
  }
})
...
router.post('/cleaning-tasks/:id/inspection-photos', async (req, res) => {
  ...
  try {
    ...
    if (parsed.data.items.length) await pgPool.query(`DELETE FROM cleaning_task_media WHERE task_id=$1 AND type LIKE 'inspection_%'`, [id])
    const uuid = require('uuid')
    for (const it of parsed.data.items) {
...
router.get('/cleaning-tasks/:id/restock-proof', async (req, res) => {
  ...
  try {
    ...
    const row = r0?.rows?.[0] || null
    if (!row) return res.status(404).json({ message: 'not found' })
    if (!await canViewMzappRestockProof(user, row, userId)) return res.status(403).json({ message: 'forbidden' })
...
router.post('/cleaning-tasks/:id/restock-proof', async (req, res) => {
  ...
  try {
    ...
    const actionAudit = await recordWorkTaskActionAudit({
      sourceType: 'cleaning_tasks',
      sourceId: String(id),
      performedAsAction: selfCompleteRestock ? 'fill_supplies' : 'submit_inspection',
      actorUserId: actionActor.actorUserId,
      performedByUserId: actionActor.performedByUserId,
      performedByName: actionActor.performedByName,
      statusBefore,
      ...
      metadata: {
        ...
        self_complete_restock: selfCompleteRestock,
        item_count: parsed.data.items.length,
...
async function expandGuestCheckoutTaskIds(seedTaskIds: string[]): Promise<string[]> {
  const seedIds = Array.from(new Set((seedTaskIds || []).map((id) => String(id || '').trim()).filter(Boolean)))
  if (!seedIds.length || !hasPg || !pgPool) return seedIds

  const seedResult = await pgPool.query(
    `SELECT id::text AS id,
            order_id::text AS order_id,
            property_id::text AS property_id,
            COALESCE(task_date, date)::date AS task_date
     FROM cleaning_tasks
     WHERE id::text = ANY($1::text[])`,
    [seedIds],
  )
  const seeds = (seedResult?.rows || []).map((row: any) => ({
    id: String(row?.id || '').trim(),
    orderId: String(row?.order_id || '').trim(),
    propertyId: String(row?.property_id || '').trim(),
    taskDate: String(row?.task_date || '').slice(0, 10),
  })).filter((row: any) => row.id)
  if (!seeds.length) return seedIds

  const orderIds = Array.from(new Set(seeds.map((row: any) => row.orderId).filter(Boolean)))
  const fallbackSeeds = seeds.filter((row: any) => !row.orderId && row.propertyId && /^\d{4}-\d{2}-\d{2}$/.test(row.taskDate))
  const propertyIds = Array.from(new Set(fallbackSeeds.map((row: any) => row.propertyId).filter(Boolean)))
  const taskDates = Array.from(new Set(fallbackSeeds.map((row: any) => row.taskDate).filter(Boolean)))
  if (!orderIds.length && !fallbackSeeds.length) return seedIds
  ...
  const relatedResult = await pgPool.query(
    `SELECT t.id::text AS id,
     ...
     WHERE ${activeCleaningTaskWhereSql('t')}
       AND lower(COALESCE(t.task_type, t.type, '')) IN ('checkin_clean', 'checkout_clean')
       AND (
         t.order_id::text = ANY($1::text[])
         OR (
           t.order_id IS NULL
           AND t.property_id::text = ANY($2::text[])
           AND COALESCE(t.task_date, t.date)::date = ANY($3::date[])
         )
       )`,
    [orderIds.length ? orderIds : ['__no_order__'], propertyIds.length ? propertyIds : ['__no_property__'], taskDates.length ? taskDates : ['1900-01-01']],
  )
  const fallbackKeys = new Set(fallbackSeeds.map((row: any) => `${row.propertyId}|${row.taskDate}`))
  const relatedIds = (relatedResult?.rows || [])
    .filter((row: any) => {
      const orderId = String(row?.order_id || '').trim()
      if (orderId && orderIds.includes(orderId)) return true
      const propertyId = String(row?.property_id || '').trim()
      const taskDate = String(row?.task_date || '').slice(0, 10)
      return !orderId && fallbackKeys.has(`${propertyId}|${taskDate}`)
    })
    .map((row: any) => String(row?.id || '').trim())
    .filter(Boolean)
  return Array.from(new Set([...seedIds, ...relatedIds]))
}
...
router.post('/cleaning-tasks/:id/guest-checked-out', async (req, res) => {
  ...
  try {
    ...
    if (action === 'unset' || action === 'clear' || action === 'cancel') {
      ...
      try {
        const { broadcastCleaningEvent } = require('./events')
        for (const taskId of taskIds) broadcastCleaningEvent({ event: 'guest_checked_out_cancelled', task_id: taskId })
      } catch {}
      try {
      ...
      await emitGuestCheckoutRealtimeEvents({
        ...
        if (propertyId) {
          ...
          await emitNotificationEvent({
            ...
            body: '房源还未退房，待退房',
            data: { entity: 'cleaning_task', entityId: String(id), action: 'open_task', kind: 'guest_checked_out_cancelled', task_id: id, task_ids: taskIds, property_code: propertyCode, checked_out_at: prevCheckedOutAt, event_id: eventId },
            actorUserId: userId,
          })
        }
      } catch {}
    ...
    try {
      const { broadcastCleaningEvent } = require('./events')
      for (const taskId of taskIds) broadcastCleaningEvent({ event: 'guest_checked_out', task_id: taskId })
    } catch {}
    try {
      const { listCleaningTaskUserIdsBulk, listManagerUserIds } = require('./notifications')
      ...
      if (propertyId) {
        ...
        await emitNotificationEvent({
          ...
          body,
          data: { entity: 'cleaning_task', entityId: String(id), action: 'open_task', kind: 'guest_checked_out', task_id: id, task_ids: taskIds, property_code: propertyCode, checked_out_at: checkedOutAt, keys_required: keysRequired, event_id: eventId },
          actorUserId: userId,
        })
      }
    } catch {}
...
router.post('/cleaning-tasks/guest-checked-out', async (req, res) => {
  ...
  try {
    ...
    if (action === 'unset' || action === 'clear' || action === 'cancel') {
      ...
      try {
        const { broadcastCleaningEvent } = require('./events')
        for (const taskId of ids2) broadcastCleaningEvent({ event: 'guest_checked_out_cancelled', task_id: taskId })
      } catch {}
      let propertyCode = ''
      let propertyId = ''
      try {
    ...
    try {
      const { broadcastCleaningEvent } = require('./events')
      for (const taskId of ids2) broadcastCleaningEvent({ event: 'guest_checked_out', task_id: taskId })
    } catch {}
...
router.post('/cleaning-tasks/order-checked-out', async (req, res) => {
  ...
  try {
    ...
    let taskIds = Array.from(new Set((rTasks?.rows || []).map((x: any) => String(x.id || '').trim()).filter(Boolean)))
    if (!taskIds.length) return res.status(404).json({ message: 'checkout task not found' })
    taskIds = await expandGuestCheckoutTaskIds(taskIds)
  ...
  try {
...

    {
      ...
      for (const x of rows) {
        ...
        out.push({
          ...
          summary: String(x.source_type || '') === 'property_maintenance'
            ? maintenanceTaskSummaryFromDetails(x.summary)
            : (x.summary !== undefined && x.summary !== null ? String(x.summary || '') : null),
          ...
          maintenance_before_photo_urls: String(x.source_type || '') === 'property_maintenance'
            ? normalizeWorkTaskPhotoUrls(x.maintenance_before_photo_urls)
            : [],
          completion_photo_urls: completionPhotoUrls,
...

    {
      ...
      if (wantCleaner || wantInspector || wantExecutor) {
        ...
        const cleaningSubmissionReadyByTaskId = new Map<string, boolean>()
        if (taskIds.length) {
          ...
          for (const row of submissionRows?.rows || []) {
            cleaningSubmissionReadyByTaskId.set(String(row.id || ''), row.cleaning_submission_ready === true)
          }
        }
        ...
        for (const row of cleaningRows) {
          ...
          const base = {
            ...
            status,
            cleaning_submission_ready: cleaningSubmissionReadyByTaskId.get(String(row.id)) === true,
            property: prop,
          }
        ...
        const buildMerged = (roleKind: 'cleaner' | 'inspector' | 'executor', rows: any[], assigneeId: string) => {
          ...
          const nextCheckinsForCheckout = p.kind === 'checkout'
            ...
            : []
          const relatedCheckoutsForCheckin = p.kind === 'checkin'
            ? cleaningRows
                .filter((x: any) => String(x?.property_id || '') === String(propId || ''))
                .filter((x: any) => cleaningType(x?.task_type) === 'checkout')
                .filter((x: any) => String(x?.task_date || x?.date || '').slice(0, 10) === date)
                .filter((x: any) => {
                  const checkinOrderId = String(p.a?.order_id || '').trim()
                  const checkoutOrderId = String(x?.order_id || '').trim()
                  return !checkinOrderId || checkinOrderId === checkoutOrderId
                })
          ...
          const relatedCheckoutRaw = String(relatedCheckoutsForCheckin[0]?.status ?? '').trim().toLowerCase()
          const relatedCheckoutProgress = relatedCheckoutRaw === 'in_progress' || relatedCheckoutRaw === 'cleaning'
            ? 'in_progress'
            : (relatedCheckoutRaw === 'cleaned' || relatedCheckoutRaw === 'restock_pending' || relatedCheckoutRaw === 'restocked')
              ? 'to_inspect'
              : ''
            ...
            roleKind === 'inspector'
              ? (
                  raw === 'keys_hung'
                    ...
                    : lockboxVideoUrl
                    ? 'keys_hung'
                    : projectInspectorTaskStatus(raw, inspectionScope) === 'to_hang_keys'
                      ? 'to_hang_keys'
    ...
    if (allowAll && (hasRole(user, 'customer_service') || hasRole(user, 'admin') || hasRole(user, 'offline_manager'))) {
      ...
      for (const [k, arr0] of byKey) {
        ...
        const cleaningStatusKey = String(cleaningStatus || '').trim().toLowerCase()
        const inspectionStatusKey = String(inspectionStatus || '').trim().toLowerCase()
        const cleaningFinished = ['done', 'completed', 'ready', 'cleaned', 'restock_pending', 'restocked', 'inspected'].includes(cleaningStatusKey)
        const inspectionPending = ['todo', 'pending', 'unassigned', 'assigned'].includes(inspectionStatusKey)
        const mergedStatusCandidates = [cleaningStatus, inspectionStatus]
          .filter((value) => String(value || '').trim())
          .sort((a: any, b: any) => rankStatus(a) - rankStatus(b))
        const statusOut =
          lockboxVideoUrl
            ? 'keys_hung'
            : cleaningFinished && inspectionPending
            ? 'to_inspect'
            : mergedStatusCandidates[0]
              || arr
                  .map((x) => x.status)
        ...
        merged.push({
          ...
          cleaning_submission_ready: cleaningSubmissionReady,
          ...
          status: statusOut,
          checked_out_at: checkedOutAtMerged || null,
...
class InternalMaintenanceFeedbackError extends Error {
  constructor(public statusCode: number, public code: string) {
    super(code)
  }
}

async function resolveInternalMaintenanceFeedbackOrigin(user: any, propertyId: string, rawSourceTaskId: any): Promise<{
  feedbackSource: InternalMaintenanceFeedbackSource
  sourceTaskId: string | null
}> {
  const sourceTaskId = String(rawSourceTaskId || '').trim()
  const isManager = canViewAll(user)
  if (!sourceTaskId) {
    if (isManager) return { feedbackSource: 'manager_feedback', sourceTaskId: null }
    throw new InternalMaintenanceFeedbackError(400, 'maintenance_feedback_source_task_required')
  }
  ...
  const sourceTask = taskResult?.rows?.[0] || null
  if (!sourceTask) throw new InternalMaintenanceFeedbackError(404, 'maintenance_feedback_source_task_not_found')
  if (String(sourceTask.property_id || '').trim() !== String(propertyId || '').trim()) {
    throw new InternalMaintenanceFeedbackError(409, 'maintenance_feedback_source_task_property_mismatch')
  }
  if (isManager) return { feedbackSource: 'manager_feedback', sourceTaskId }

  const actorId = String(user?.sub || '').trim()
  const cleanerId = String(sourceTask.cleaner_id || '').trim()
  const inspectorId = String(sourceTask.inspector_id || '').trim()
  const assigneeId = String(sourceTask.assignee_id || '').trim()
  if (!actorId || ![cleanerId, inspectorId, assigneeId].includes(actorId)) {
    throw new InternalMaintenanceFeedbackError(403, 'maintenance_feedback_source_task_forbidden')
  }
  const isInspectorFeedback = inspectorId === actorId || (assigneeId === actorId && !cleanerId && (isInspectorRole(user) || isCleanerInspectorRole(user)))
  return { feedbackSource: isInspectorFeedback ? 'inspection_feedback' : 'cleaning_feedback', sourceTaskId }
}

async function createInternalMaintenanceFromFeedback(input: {
  id: string
  propertyId: string
  occurredAt: string
  details: string
  ...
  try {
    ...
    const created = await client.query(
      ...
      [
        ...
        input.workNo,
        input.area,
        input.invoiceDescriptionEn || null,
        input.fingerprint,
        input.feedbackSource,
        input.sourceTaskId,
        input.categoryDetail || null,
        input.clientItemId || null,
      ],
    )
    const row = created?.rows?.[0] || null
    if (!row) throw new InternalMaintenanceFeedbackError(500, 'maintenance_feedback_create_failed')
    await upsertMaintenanceWorkTask(client, 'internal', row)
...
class PropertyDeepCleaningSchemaNotReady extends Error {
  constructor() {
    super('property_deep_cleaning_schema_not_ready')
    this.name = 'PropertyDeepCleaningSchemaNotReady'
  }
}

/** The controlled migration owns DDL; request paths only verify the required projection. */
async function ensurePropertyDeepCleaningColumns(client: any = pgPool) {
  if (!client) throw new PropertyDeepCleaningSchemaNotReady()
  try {
    ...
    throw new PropertyDeepCleaningSchemaNotReady()
  }
}
...
async function syncFeedbackWorkTask(kind: FeedbackKind, id: string, status: string, client: any = pgPool) {
  if (!client) return
  ...
  if (workStatus === 'resolved' || workStatus === 'cancelled') {
    await client.query(`DELETE FROM work_tasks WHERE source_type = $1 AND source_id = $2`, [sourceType, id])
    return
  }
  await client.query(`UPDATE work_tasks SET status = $1, updated_at = now() WHERE source_type = $2 AND source_id = $3`, [workStatus === 'in_progress' ? 'in_progress' : 'todo', sourceType, id])
}
  ...
  try {
    ...
    if (parsed.data.kind === 'maintenance') {
      ...
      try {
        ...
        if (error instanceof InternalMaintenanceFeedbackError) {
      ...
      const photoCast = photoType === 'jsonb' ? '::jsonb' : photoType === 'text[]' ? '::text[]' : ''
      ...
      if (items) {
        ...
        for (const it of prepared) {
          ...
          await createInternalMaintenanceFromFeedback({
            ...
            workNo,
            area: it.area,
            categoryDetail: it.category || null,
            invoiceDescriptionEn: invoiceDescriptionEn || null,
            fingerprint,
            clientItemId: null,
      ...
      const categoryDetail = String((parsed.data as any).category || '').trim()
    ...
    const invoiceDescriptionEn = String((parsed.data as any).invoice_description_en || '').trim()
    if (!detail) return res.status(400).json({ message: 'missing detail' })
    await ensurePropertyDeepCleaningColumns(pool)
...
async function loadPropertyFeedbackRow(kind: FeedbackKind, id: string, client: any = pgPool, lockForUpdate = false) {
  if (!client) return null
...
async function ensurePropertyFeedbackKindColumns(kind: PropertyFeedbackRecordKind, client: any = pgPool) {
  if (kind === 'maintenance') return ensurePropertyMaintenanceColumns()
  if (kind === 'deep_cleaning') return ensurePropertyDeepCleaningColumns(client)
...
router.post('/property-feedbacks/:kind/:id/move', async (req, res) => {
  ...
  if (sourceKind === 'maintenance') return res.status(409).json({ code: 'maintenance_workflow_action_required' })
  if (!id) return res.status(400).json({ message: 'missing id' })
...
async function persistFeedbackProjects(
  kind: FeedbackKind,
  row: any,
  items: PropertyFeedbackProjectItem[],
  options?: { maintenanceCompletion?: { actorUserId: string; actorName: string } },
  transactionClient?: any,
) {
  if (!pgPool) throw new Error('pg not available')
  const summary = summarizeProjectItems(kind, items)
  const currentMaintenanceStatus = kind === 'maintenance'
    ? normalizeMaintenanceWorkflowStatus(row?.status, row?.review_status)
    : null
  const shouldSubmitMaintenanceFeedback = kind === 'maintenance'
    && !!options?.maintenanceCompletion
    && summary.status === 'completed'
    && currentMaintenanceStatus !== 'pending_review'
    && currentMaintenanceStatus !== 'closed'
  const status = kind === 'maintenance'
    ? (shouldSubmitMaintenanceFeedback ? 'pending_review' : String(row?.status || '').trim() || 'pending_assignment')
    : projectStatusFromItems(summary.items)
  const sourceTable = kind === 'maintenance' ? 'property_maintenance' : 'property_deep_cleaning'
  const reviewStatus = kind === 'maintenance'
    ? (shouldSubmitMaintenanceFeedback ? 'pending' : (row?.review_status ? String(row.review_status) : null))
    : (status === 'completed' ? 'pending' : (row?.review_status ? String(row.review_status) : 'pending'))
  const completedAt = kind === 'maintenance'
    ? (shouldSubmitMaintenanceFeedback ? (summary.completed_at || new Date().toISOString()) : (row?.completed_at || null))
  ...
  const ownsTransaction = shouldSubmitMaintenanceFeedback && !transactionClient
  const client = transactionClient || (ownsTransaction ? await pgPool.connect() : null)
  ...
  if (kind === 'maintenance') {
    ...
    const completionType = await getColumnType(sourceTable, 'completion_photo_urls')
    const beforeExpr = beforeType === 'text[]' ? '$4::text[]' : '$4::jsonb'
    const afterExpr = afterType === 'text[]' ? '$5::text[]' : '$5::jsonb'
    const completionExpr = completionType === 'text[]' ? '$7::text[]' : '$7::jsonb'
    const updatedResult = await queryClient.query(
      `UPDATE property_maintenance
          SET project_items = $2::jsonb,
              ...
              repair_photo_urls = ${afterExpr},
              repair_notes = $6,
              completion_photo_urls = ${completionExpr},
              status = $8,
      ...
      [
        ...
        completionType === 'text[]' ? summary.repair_photo_urls : afterJson,
        status,
        completedAt,
        reviewStatus,
      ],
    ...
    if (!updatedRow) throw new Error('maintenance_feedback_project_update_failed')
    if (shouldSubmitMaintenanceFeedback && client && options?.maintenanceCompletion) {
      await upsertMaintenanceWorkTask(client, 'internal', updatedRow)
      await insertMaintenanceWorkflowEvent(client, {
        domain: 'internal',
        recordId: String(row.id),
        eventType: 'feedback_completed',
        fromStatus: currentMaintenanceStatus || 'pending_assignment',
        toStatus: 'pending_review',
        actorUserId: options.maintenanceCompletion.actorUserId,
        actorName: options.maintenanceCompletion.actorName,
        reason: null,
        payload: {
          feedback_source: row?.feedback_source || 'legacy',
          source_task_id: row?.source_task_id || null,
          completion_photo_count: summary.repair_photo_urls.length,
        },
      })
    }
  } else {
  ...
  if (ownsTransaction) await client.query('COMMIT')
  if (kind !== 'maintenance') await syncFeedbackWorkTask(kind, String(row.id), status, queryClient)
  return loadPropertyFeedbackRow(kind, String(row.id), queryClient)
  } catch (error) {
...
router.post('/property-feedbacks/:kind/:id/projects', async (req, res) => {
  ...
  try {
    ...
    const receiptScope = submitId && stepKey
      ? {
          scopeType: 'property_feedback_project_create',
          scopeId: `${id}:${clientItemId || 'server'}`,
          submitId,
          stepKey,
        }
      : null
    const response = await pgRunInTransaction(async (client) => {
      ...
      if (receiptScope) {
        ...
        if (receipt) {
          if (String(receipt.payload_hash || '') !== payloadHash) {
            return { statusCode: 409, body: { message: 'idempotency_conflict', submit_id: submitId, step_key: stepKey, client_item_id: clientItemId || null } }
          }
          return { statusCode: 200, body: receipt.response_json || { ok: true } }
        }
      }
      const current = summarizeProjectItems(kind, row.project_items, row).items
      const requestedArea = String(parsed.data.area || '').trim()
      const requestedCategory = kind === 'maintenance' ? String(parsed.data.category || '').trim() : ''
      const requestedDetail = kind === 'maintenance' ? String(parsed.data.detail || '').trim() : ''
      const existingOpenMaintenanceProject = kind === 'maintenance' && requestedArea && requestedDetail
        ? current.find((candidate: PropertyFeedbackProjectItem) => (
          candidate.status !== 'completed'
          && String(candidate.area || '').trim() === requestedArea
          && String(candidate.category || '').trim() === requestedCategory
          && String(candidate.detail || '').trim() === requestedDetail
        ))
        : null
      if (existingOpenMaintenanceProject) {
        const body = { ok: true, item: existingOpenMaintenanceProject, row }
        if (receiptScope) await saveIdempotentStepReceipt(client, receiptScope, payloadHash, body)
        return { statusCode: 200, body }
      }
      const item: PropertyFeedbackProjectItem = {
        id: clientItemId || require('uuid').v4(),
        name: String(parsed.data.name || '').trim(),
        area: requestedArea || null,
      ...
      const updated = await persistFeedbackProjects(kind, row, [...current, item], undefined, client)
      const body = { ok: true, item, row: updated }
      if (receiptScope) await saveIdempotentStepReceipt(client, receiptScope, payloadHash, body)
      return { statusCode: 201, body }
    })
    if (!response) throw new Error('property_feedback_project_create_transaction_failed')
...
router.post('/property-feedbacks/:kind/:id/projects/:projectId/complete', async (req, res) => {
  ...
  try {
    ...
    const receiptScope = submitId && stepKey
      ? {
          scopeType: 'property_feedback_project_complete',
          scopeId: `${id}:${projectId}:${clientItemId || 'server'}`,
          submitId,
          stepKey,
        }
      : null
    const response = await pgRunInTransaction(async (client) => {
      ...
      const maintenanceCompletion = kind === 'maintenance'
        ? await resolveInternalMaintenanceFeedbackOrigin(user, String(row.property_id || '').trim(), row.source_task_id)
        : null
      const current = summarizeProjectItems(kind, row.project_items, row).items
      const found = current.find((it: PropertyFeedbackProjectItem) => it.id === projectId)
      if (!found) return { statusCode: 404, body: { message: 'project_not_found' } }
      ...
      if (!afterPhotos.length) return { statusCode: 400, body: { message: 'missing after_photos' } }
      if (kind === 'deep_cleaning') {
        const startedAt = toIsoOrNull(parsed.data.started_at)
        const endedAt = toIsoOrNull(parsed.data.ended_at)
        if (!startedAt || !endedAt) return { statusCode: 400, body: { message: 'missing started_or_ended_at' } }
        if (endedAt < startedAt) return { statusCode: 400, body: { message: 'ended_before_started' } }
        if (!beforePhotos.length) return { statusCode: 400, body: { message: 'missing before_photos' } }
      ...
      const updated = await persistFeedbackProjects(kind, row, current, maintenanceCompletion ? {
        maintenanceCompletion: {
          actorUserId: String(user.sub || '').trim() || 'unknown',
          actorName: String(user.username || user.sub || '').trim() || 'unknown',
        },
      } : undefined, client)
      const body = { ok: true, item: found, row: updated }
      if (receiptScope) await saveIdempotentStepReceipt(client, receiptScope, payloadHash, body)
      return { statusCode: 200, body }
    })
    if (!response) throw new Error('property_feedback_project_complete_transaction_failed')
    return res.status(response.statusCode).json(response.body)
  } catch (e: any) {
    if (e instanceof InternalMaintenanceFeedbackError) return res.status(e.statusCode).json({ code: e.code })
    if (e instanceof IdempotentStepReceiptsNotReady) return res.status(503).json({ code: e.message })
    if (e instanceof PropertyDeepCleaningSchemaNotReady) return res.status(503).json({ code: e.message })
