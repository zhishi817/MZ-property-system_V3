...
router.get('/inbox', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: String(e?.message || 'inbox_failed') })
  }
})
...
router.get('/unread-count', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: String(e?.message || 'unread_count_failed') })
  }
})
