...
- root/CRL-20260907-001：已关闭未合并的当前-Dev 对齐来源候选。
- root/CRL-20260909-002：基于当前 Dev 的重提取候选。
...
- **后端：** `PATCH /orders/:id` 的更新校验、取消权限/结算锁定/重复确认码/安全错误响应；不改变导入、同步、历史回填或数据库 schema。
- **Web：** `/orders` 列表稳定排序、取消订单日历显示、编辑抽屉错误展示和详情到日历的直接导航。
- **入口：** 手动编辑、自动取消后的列表/日历刷新、订单详情抽屉。
- **一致性：** 订单时间与取消状态由后端保存，前端只按该记录显示并保持确定性排序；财务是否计入收入不再决定日历记录是否存在。
...
- 订单导入、自动邮件同步、清洁任务执行、财务金额规则、历史订单批量修复和移动端显示。
- 未经明确授权的推送、部署、生产 API 调用、生产数据修改或真机验证。
...
| 关闭后修正 Web 请求与普通字段隔离 | `frontend/src/lib/maintenanceWorkflowActions.test.ts` | 专用修正动作发送实际完成日期、实际维修人员、维修后照片、备注、原因与幂等键，不走通用 PATCH；表单初始化的既有金额和扣款方式不得阻止仅修改完成日期，真实普通字段变更必须被识别并隔离保存 | sufficient | `npm run test --prefix frontend -- --run src/lib/maintenanceWorkflowActions.test.ts --coverage.enabled=false` |
...
- 付款截止日必须是账单所属月的真实最后一天；系统不得把“30 号”当作所有月份的固定日期。
- 待处理日历的事件日期按 `实际收到账单日 > 预计收到账单日 > 无日期` 决定。没有实际或预计收账日的账单必须进入待安排计数，不得被虚构为月底事项。
- 已付账单不得出现在待处理收账日历；它们只保留在既有已付记录中。
- 实际到账日优先于预计到账日，且未付款账单在该优先日期已过时必须成为 `payment_overdue`；此规则不依赖金额已确认。已付账单不能逾期。
- 月历格子只能展示当天实际/预计到账的汇总和清晰的逾期数量；不得把无日期、未确认账单批量集中到月末，也不得把同一逾期账单复制到月末。
- 月历首尾可见的相邻月份日期也必须按其真实到账日显示摘要和逾期；点击该日期必须打开原账单，而不是复制成当前月份账单。
- 当天处理区必须服务端分页、可按房源/收费公司/账号搜索、保留对应 `month_key`，并使用与左侧队列一致的完整账单卡片；逾期卡片必须排在当天其他卡片之前，不能用窄表格截断日期或状态。

### 跨层适用范围

- **后端：** `GET /recurring/property-payables/workbench` 的月末和日历元数据，新增只读 `/calendar` 与 `/calendar-day`，以及既有确认账单/上传发票/登记支付接口使用的账单月份。
- **客户端：** `/finance/property-payables` 既有处理队列、月历和右侧当天卡片面板；模板及房源详情对月末截止日的文案。
- **入口：** 待处理与日历视图、当天“处理”按钮、已有账单详情抽屉、已付记录。
- **一致性：** 收账事件日、付款风险日和账单所属月是不同字段；客户端不使用 `due_date` 重新推导日历事件日。

### 测试映射

| 保护点 | 测试文件 | 测试场景 | 覆盖状态 | 执行命令 |
|---|---|---|---|---|
| 账单月真实最后一天、实际优先、预计兜底、到账日已过即未付款逾期、无日期不落月末、已付不进待处理日历 | `backend/scripts/tests/test_property_payable_bill_dates.ts` | 2 月及 31 日 clamp；实际日期覆盖预计日期；仅预计日期可排期；实际/预计到账日的逾期边界；无日期/已付返回 `unscheduled` | not-wired | `./node_modules/.bin/ts-node-dev --transpile-only scripts/tests/test_property_payable_bill_dates.ts` (in `backend`) |
| 新日历 API 与现有页面的类型集成 | `frontend/src/app/finance/property-payables/page.tsx` | 月度摘要按到账日期标示逾期；可见的上/下月格子同时读取其真实月份摘要，点击后按原 `month_key` 打开；当天服务端分页/搜索、同左侧的完整卡片和跨月操作路径由 production build 检查 | partial | `npm run build --prefix frontend` |

### 验证策略

- **代码验证：** 日期纯规则测试、后端 TypeScript 构建、前端生产构建、registry/ledger 审计和 diff 检查。
- **非生产集成：** 使用授权测试账号建立三笔独立账单：仅预计日、预计后填实际日、无日期；确认它们分别显示在预计日、实际日和待安排。将当前日期推进到到账日之后，核对未付款卡片变红并优先；查看相邻月份时，核对首尾可见的上/下月格子保留该日期的同一摘要及可点击账单；再登记支付，核对逾期消失。
- **生产：** 不得为验证本规则创建、确认、上传或支付真实账单；任何历史数据修复均需单独只读预览和明确写入授权。

### 最后验证

- **CRL：** CRL-20260901-001
- **Commit：** not committed
- **日期：** 2026-09-01
...
- 模板的付款方式、收款账户、发票存储、已付历史、支付权限和历史账单批量补日期；任何生产账单的确认、上传、付款或数据修复。
