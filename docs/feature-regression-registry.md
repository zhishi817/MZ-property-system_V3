...
| 线下任务执行人 source/canonical 一致与角色边界 | `backend/scripts/tests/test_offline_task_assignment_patch.ts` | 客服、线下经理和 admin 更新执行人后源任务与 canonical `work_tasks` 一致；普通编辑不清空，null 返回业务错误 | sufficient | `npm run test:offline-task-assignment-patch --prefix backend` |
