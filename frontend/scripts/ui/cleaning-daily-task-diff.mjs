...
const staticDir = path.join(root, 'ui', 'cleaning-daily-task-static')
const staticHtml = path.join(staticDir, 'index.html')

const baselineDir = path.join(root, 'ui', 'cleaning-daily-task-baseline')
const outDir = path.join(root, 'ui', 'cleaning-daily-task-report')

const viewports = [
  { name: '1920x400', width: 1920, height: 400 },
  { name: '1366x400', width: 1366, height: 400 },
]
...
async function screenshotOnce(browserName, launcher, vp) {
  ...
  try {
    await page.waitForFunction(() => (window).__CLEANING_DAILY_TASK_READY__ === true, null, { timeout: 5000 })
  } catch {
...
function writeReportMd(rows, overallOk) {
  const md = [
    '# 清洁当日任务卡片 1:1 还原度对照报告',
    '',
    `- 基准：${updateBaseline ? '已更新（--update-baseline）' : 'ui/cleaning-daily-task-baseline 下的 PNG（建议放设计导出或截图）'}`,
...
async function main() {
  ...
  const jsonOut = results.map((r) => ({
    ...r,
    actualPath: r.actualPath ? path.relative(root, r.actualPath) : r.actualPath,
    diffPath: r.diffPath ? path.relative(root, r.diffPath) : r.diffPath,
  }))
