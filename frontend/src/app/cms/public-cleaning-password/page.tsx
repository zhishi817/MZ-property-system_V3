...
export default function Page() {
  ...
  async function load() {
    try {
      const res = await fetch(`${API_BASE}/public/cleaning-guide/password-info`, { headers: authHeaders() })
  ...
  async function reset() {
    try {
      const res = await fetch(`${API_BASE}/public/cleaning-guide/reset-password`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders() }, body: JSON.stringify({ new_password: pwd }) })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
  ...
  return (
    <div>
      <Typography.Title level={3}>清洁公开访问密码</Typography.Title>
