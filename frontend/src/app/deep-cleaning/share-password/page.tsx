...
export default function Page() {
  ...
  async function load() {
    try {
      const res = await fetch(`${API_BASE}/public/deep-cleaning-share/password-info`, { headers: authHeaders() })
  ...
  async function reset() {
    try {
      const res = await fetch(`${API_BASE}/public/deep-cleaning-share/reset-password`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders() }, body: JSON.stringify({ new_password: pwd }) })
      if (!res.ok) {
  ...
  return (
    <div>
      <Typography.Title level={3}>深度清洁外链访问密码</Typography.Title>
