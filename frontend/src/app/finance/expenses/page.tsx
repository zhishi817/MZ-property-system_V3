...
export default function ExpensesPage() {
  ...
  async function load() {
    const tx = await getJSON<Tx[]>('/finance')
    setList((tx || []).filter(t => t.kind === 'expense'))
  }
  ...
  async function submit() {
    ...
    const url = editing ? `${API_BASE}/finance/${editing.id}` : `${API_BASE}/finance`
    const method = editing ? 'PATCH' : 'POST'
    const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json', ...authHeaders() }, body: JSON.stringify(payload) })
    if (res.ok) { message.success(editing ? '已更新支出' : '已记录支出'); form.resetFields(); setOpen(false); setEditing(null); load() }
    else {
      try { const err = await res.json(); message.error(err?.message || `提交失败 (${res.status})`) } catch { message.error(`提交失败 (${res.status})`) }
    }
  }
      ...
      <Space>
        ...
        <Button danger onClick={() => {
          modal.confirm({ title: '确认删除支出', okType: 'danger', onOk: async () => {
            const res = await fetch(`${API_BASE}/finance/${r.id}`, { method: 'DELETE', headers: { ...authHeaders() } })
  ...
  return (
    <Card title="支出管理" extra={hasPerm('finance.payout') ? <Button type="primary" onClick={() => { setEditing(null); form.resetFields(); setOpen(true) }}>记录支出</Button> : null}>
      ...
      <Table rowKey={r => r.id} columns={columns as any} dataSource={list.filter(x => {
        ...
        return codeOk && catOk && inRange
      })} pagination={{ pageSize: 10 }} scroll={{ x: 'max-content' }} />
      <Modal open={open} onCancel={() => setOpen(false)} onOk={submit} title="记录支出">
