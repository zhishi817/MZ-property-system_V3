...
export default function ConsumableItemsPriceListView() {
  ...
  async function load() {
    setLoading(true)
    try {
      const data = await getJSON<PriceRow[]>('/inventory/consumable-items-prices')
      setRows(data || [])
    } catch (e: any) {
  ...
  async function submit() {
    try {
      ...
      if (!editingRow) {
        await postJSON('/inventory/consumable-items-prices', payload)
        message.success('已新增')
      } else {
        await patchJSON(`/inventory/consumable-items-prices/${editingRow.id}`, payload)
        message.success('已更新')
      }
      setFormOpen(false)
      setEditingRow(null)
  ...
  return (
    <>
      <Card title="消耗品价格表" extra={canManage ? <Button type="primary" onClick={openCreate}>新增消耗品</Button> : null}>
      ...
      >
        {detailRow ? (
          <Descriptions title="消耗品基础信息" bordered column={2} labelStyle={{ width: '120px' }}>
