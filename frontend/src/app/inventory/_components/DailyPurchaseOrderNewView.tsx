...
function sortDailyItems(rows: DailyItem[]) {
...
export default function DailyPurchaseOrderNewView() {
  ...
  async function loadBase() {
    const [ws, ss, dailyItems] = await Promise.all([
    ...
    const activeSuppliers = (ss || []).filter((row) => row.active && row.kind === 'daily')
    const activeItems = sortDailyItems((dailyItems || []).filter((row) => row.is_active !== false))
    ...
    const preselectedIds = activeItems
      .filter((item) => Number(currentLines?.[item.id]?.quantity || 0) > 0)
      .map((item) => item.id)
    form.setFieldsValue({
    ...
    if (preselectedIds.length) setSelectedItemIds(preselectedIds)
  }

  useEffect(() => {
    loadBase().catch((e) => message.error(e?.message || '加载失败'))
  }, [])
    ...
    try {
      ...
      const lines = items
        .map((item) => {
          ...
          return {
            item_id: `item.daily_price.${item.id}`,
            quantity,
            unit: item.unit || 'pcs',
            unit_price: Number(item.cost_unit_price || 0),
          }
        })
        .filter(Boolean)
      if (!lines.length) throw new Error('请至少填写一种日用品数量')

      const result = await postJSON<any>('/inventory/purchase-orders', {
      ...
      message.success('日用品采购单已创建')
      const poId = String(result?.po?.id || '')
      if (poId && typeof window !== 'undefined') window.location.href = `/inventory/purchase-orders/${poId}?category=daily`
    } catch (e: any) {
  ...
  const columns = [
    ...
    { title: '采购价', dataIndex: 'unitPrice', width: 120, align: 'right' as const, render: (value: number) => `$${Number(value || 0).toFixed(2)}` },
    {
      ...
      width: 140,
      render: (_: any, row: any) => (
        <Form.Item name={['linesByItem', row.id, 'quantity']} style={{ marginBottom: 0 }}>
    ...
    { title: '金额', dataIndex: 'amount', width: 120, align: 'right' as const, render: (value: number) => `$${Number(value || 0).toFixed(2)}` },
    {
      title: '操作',
      width: 100,
      render: (_: any, row: any) => <Button danger onClick={() => removeItem(String(row.id))}>删除</Button>,
    },
  ]
  ...
  return (
    <Card
      ...
      extra={
        <Space>
          <Link href="/inventory/category/daily/purchase-orders" prefetch={false}>
            <Button icon={<ArrowLeftOutlined />}>返回采购记录</Button>
          </Link>
          <Button type="primary" loading={submitting} onClick={() => submit().catch(() => {})}>保存采购单</Button>
        </Space>
      }
    >
      <Form form={form} layout="vertical">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(180px, 1fr))', gap: 16, marginBottom: 20 }}>
          <Form.Item name="supplier_id" label="供应商" rules={[{ required: true, message: '请选择供应商' }]}>
            <Select options={supplierOptions} placeholder="请选择日用品供应商" />
          </Form.Item>
          <Form.Item name="warehouse_id" label="送货仓库" rules={[{ required: true, message: '请选择仓库' }]}>
            <Select options={whOptions} placeholder="请选择仓库" />
          </Form.Item>
        ...
        <div style={{ padding: 16, border: '1px solid #f0f0f0', borderRadius: 12, background: '#fafafa', marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', gap: 12, alignItems: 'start' }}>
            ...
            <div>
              ...
              <Button type="primary" onClick={addItem}>添加物品</Button>
            </div>
          </div>
          <div style={{ marginTop: 6, color: '#8c8c8c', fontSize: 12 }}>
            输入关键字后可多选日用品，再点“添加物品”统一加入采购单
          </div>
        </div>

        {itemRows.length ? (
          <Table rowKey="id" columns={columns as any} dataSource={itemRows} pagination={false} scroll={{ x: 1000 }} />
        ) : (
          <div style={{ border: '1px dashed #d9d9d9', borderRadius: 12, padding: 32, background: '#fff' }}>
            <Empty description="输入并选择日用品后，再添加到采购单" />
          </div>
        )}

        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16 }}>
          <Space direction="vertical" size={4} style={{ minWidth: 220 }}>
            <Typography.Text>未税金额：${totalAmount.toFixed(2)}</Typography.Text>
            <Typography.Text>GST：${gstAmount.toFixed(2)}</Typography.Text>
            <Typography.Text strong>含税总额：${totalAmountInclGst.toFixed(2)}</Typography.Text>
          </Space>
        </div>
