...
import { Button, Card, Empty, Form, Input, InputNumber, Select, Space, Table, Typography, DatePicker, message } from 'antd'
...
type OtherItem = { id: string; sku: string; item_name: string; unit?: string | null; default_quantity?: number | null; cost_unit_price?: number | null; unit_price?: number | null; currency?: string | null; is_active?: boolean; sort_order?: number | null }

export default function OtherPurchaseOrderNewView() {
  ...
  async function loadBase() {
    const [ws, ss, otherItems] = await Promise.all([
      getJSON<Warehouse[]>('/inventory/warehouses'),
      getJSON<Supplier[]>('/inventory/suppliers'),
      getJSON<OtherItem[]>('/inventory/other-items-prices'),
    ])
    const activeWarehouses = (ws || []).filter((row) => row.active)
    const activeSuppliers = (ss || []).filter((row) => row.active && row.kind === 'other')
    const activeItems = (otherItems || []).filter((row) => row.is_active !== false)
  ...
  async function submit() {
    ...
    try {
      ...
      const lines = items.map((item) => {
        ...
        return { item_id: `item.other_price.${item.id}`, quantity, unit: item.unit || 'pcs', unit_price: Number(item.cost_unit_price || 0) }
      }).filter(Boolean)
      if (!lines.length) throw new Error('请至少填写一种其他物品数量')
      ...
      message.success('其他物品采购单已创建')
      const poId = String(result?.po?.id || '')
      if (poId && typeof window !== 'undefined') window.location.href = `/inventory/purchase-orders/${poId}?category=other`
    } catch (e: any) {
  ...
  const columns = [
    { title: '其他物品', width: 240, render: (_: any, row: any) => <div><div style={{ fontWeight: 500 }}>{row.item_name}</div><div style={{ color: '#8c8c8c', fontSize: 12 }}>{row.sku}</div></div> },
  ...
  return (
    <Card title="新建其他物品采购单" extra={<Space><Link href="/inventory/category/other/purchase-orders" prefetch={false}><Button icon={<ArrowLeftOutlined />}>返回采购记录</Button></Link><Button type="primary" loading={submitting} onClick={() => submit().catch(() => {})}>保存采购单</Button></Space>}>
      <Form form={form} layout="vertical">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(180px, 1fr))', gap: 16, marginBottom: 20 }}>
          <Form.Item name="supplier_id" label="供应商" rules={[{ required: true, message: '请选择供应商' }]}><Select options={supplierOptions} placeholder="请选择其他物品供应商" /></Form.Item>
        ...
        <div style={{ padding: 16, border: '1px solid #f0f0f0', borderRadius: 12, background: '#fafafa', marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', gap: 12, alignItems: 'start' }}>
            <div>
              ...
              <div style={{ marginTop: 6, color: '#8c8c8c', fontSize: 12 }}>输入关键字后可多选其他物品，再点“添加物品”统一加入采购单</div>
            </div>
            <div><div style={{ marginBottom: 8, fontWeight: 500, visibility: 'hidden' }}>操作</div><Button type="primary" onClick={addItem}>添加物品</Button></div>
          </div>
        </div>

        {itemRows.length ? <Table rowKey="id" columns={columns as any} dataSource={itemRows} pagination={false} scroll={{ x: 920 }} /> : <div style={{ border: '1px dashed #d9d9d9', borderRadius: 12, padding: 32, background: '#fff' }}><Empty description="输入并选择其他物品后，再添加到采购单" /></div>}
