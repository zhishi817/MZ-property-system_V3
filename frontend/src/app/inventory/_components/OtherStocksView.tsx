...
import { App, Button, Card, DatePicker, Descriptions, Drawer, Empty, Form, Input, InputNumber, Select, Space, Table } from 'antd'
...
type OtherStockOverview = { warehouses: Warehouse[]; items: OtherStockItem[] }
type StocktakeRow = {
  id: string
  warehouse_id: string
...
export default function OtherStocksView() {
  const { message } = App.useApp()
  const [loading, setLoading] = useState(false)
  const [warehouses, setWarehouses] = useState<Warehouse[]>([])
  const [rows, setRows] = useState<OtherStockItem[]>([])
  ...
  async function load() {
    ...
    try {
      const [overviewResult, stocktakeResult] = await Promise.allSettled([
        getJSON<OtherStockOverview>('/inventory/other-stock-overview'),
        getJSON<StocktakeRow[]>('/inventory/stocktakes?category=other&limit=10'),
      ])
  ...
  const stockColumns = useMemo(() => {
    const warehouseColumns = warehouses.map((warehouse) => ({
      ...
      render: (_: any, row: OtherStockItem) => {
    ...
    return [
      {
        title: '其他物品',
        width: 240,
        render: (_: any, row: OtherStockItem) => <div><div style={{ fontWeight: 500 }}>{row.item_name}</div><div style={{ marginTop: 4 }}><span style={{ color: '#8c8c8c', fontSize: 12 }}>{row.sku}</span></div></div>,
      },
  ...
  const stocktakeEditorColumns: any[] = [
    { title: '其他物品', render: (_: any, row: any) => <div><div style={{ fontWeight: 500 }}>{row.item_name}</div><div style={{ color: '#8c8c8c', fontSize: 12 }}>{row.sku}</div></div> },
    { title: '单位', dataIndex: 'unit', width: 90, render: (value: string) => value || '-' },
    { title: '当前库存', width: 100, render: (_: any, row: OtherStockItem) => Number(row.stock_by_warehouse.find((entry) => entry.warehouse_id === String(form.getFieldValue('warehouse_id') || ''))?.quantity || 0) },
  ...
  return (
    <>
      <Card title="其他物品库存" extra={<Space><Button onClick={() => load().catch(() => {})}>刷新</Button><Button onClick={() => openStocktake('initial')}>初始化库存</Button><Button type="primary" icon={<PlusOutlined />} onClick={() => openStocktake('routine')}>库存更新</Button></Space>}>
      ...
      <Drawer title={stocktakeType === 'initial' ? '初始化其他物品库存' : '更新其他物品库存'} placement="right" width={860} open={stocktakeOpen} onClose={() => setStocktakeOpen(false)} extra={<Space><Button onClick={() => setStocktakeOpen(false)}>取消</Button><Button type="primary" loading={submitting} onClick={() => submitStocktake().catch(() => {})}>保存</Button></Space>}>
        <Form form={form} layout="vertical">
          ...
          <div style={{ padding: 16, border: '1px solid #f0f0f0', borderRadius: 12, background: '#fafafa', marginBottom: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) auto', gap: 12, alignItems: 'start' }}>
              <div>
                ...
                <Select mode="multiple" showSearch value={pendingItemIds} onChange={(value) => setPendingItemIds((value || []).map((item) => String(item)))} options={itemOptions} placeholder="输入其他物品名称或 SKU 搜索，可多选" optionFilterProp="label" optionLabelProp="label" style={{ width: '100%' }} notFoundContent="没有可添加的其他物品" />
              </div>
          ...
          {selectedRows.length ? <Table rowKey="item_id" columns={stocktakeEditorColumns} dataSource={selectedRows} pagination={false} scroll={{ x: 760 }} /> : <div style={{ border: '1px dashed #d9d9d9', borderRadius: 12, padding: 32, background: '#fff' }}><Empty description="先选择其他物品，再填写盘点数量" /></div>}
        </Form>
      </Drawer>
        ...
        {detail ? (
          <div style={{ display: 'grid', gap: 16 }}>
            ...
            <Table rowKey={(row) => row.id} pagination={false} columns={[
              { title: '其他物品', dataIndex: 'item_name' },
