"use client"
import { Card, Table, Space, Button, Input, Select, DatePicker, Modal, Form, App, Upload } from 'antd'
...
export default function MaintenanceRecordsUnified() {
  ...
  async function save() {
    ...
    try {
      ...
      message.success('已更新记录'); setOpen(false); setEditing(null); load()
    } catch (e: any) {
    ...
    <Space direction="vertical" style={{ width: '100%' }}>
      ...
      <Modal open={open} onCancel={()=>setOpen(false)} onOk={save} title="更新维修记录状态" okText="保存">
        <Form form={form} layout="vertical">
          ...
          {String(Form.useWatch('status', form) || '') === 'completed' ? (
