...
export async function runWorkRecordPdfJob(opts: {
  createPath: string
  ...
  onUpdate({ open: true, stage: '创建任务', detail: '正在创建 PDF 任务...', progress: 5, timeout: false })
  ...
  while (Date.now() - startedAt < 8 * 60 * 1000) {
    ...
    pollCount += 1
    try {
      const st = await fetchJsonWithTimeout(`${API_BASE}${opts.statusPath(jobId)}`, { headers: authHeaders() }, 20000)
      if (!st.resp.ok) continue
      const s = (st.json || {}) as PdfJobStatus
      ...
      if (String(s.status || '') === 'failed' || stage === 'failed') {
        throw new Error(String(s.last_error_message || s.detail || 'PDF 生成失败'))
      }
      if (String(s.status || '') === 'success' && stage === 'done') {
        onUpdate({ stage: '准备下载', detail: 'PDF 已生成，正在下载...', progress: 96, timeout: false })
        const dl = await fetchBlobWithTimeout(`${API_BASE}${opts.downloadPath(jobId)}`, { headers: authHeaders() }, 30000)
        if (!dl.resp.ok) {
  ...
  onUpdate({ timeout: true, stage: '生成时间较长', detail: 'PDF 仍在后台处理中，请稍后重试。' })
  throw new Error('PDF 生成时间较长，请稍后重试')
}
